...
export default function OverlappingImages({
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (containerTop < windowHeight && containerTop + containerHeight > 0) {
        ...
        // Horizontal movement for more dynamic effect
  ...
  return (
    <div ref={containerRef} className="relative w-full h-[550px] md:h-[600px] lg:h-[650px] overflow-hidden">
      ...
      <div 
        ref={overlayRef}
        className={`absolute top-4 right-2 sm:top-6 sm:right-6 lg:top-8 lg:right-8 w-40 h-[240px] sm:w-64 sm:h-[320px] lg:w-80 lg:h-[420px] bg-white/10 z-0 shadow-2xl backdrop-blur-sm will-change-transform transition-all duration-1000 ${
          hasAnimated ? 'opacity-100 scale-100' : 'opacity-0 scale-95'
        }`}
      ></div>
      
      {/* Secondary Image (behind, bottom right) */}
      {secondaryImage && (
        <div
          ref={secondaryRef}
          className={`absolute bottom-0 right-2 md:right-4 lg:right-12 w-[360px] h-[280px] md:w-[420px] md:h-[320px] lg:w-[520px] lg:h-[380px] overflow-hidden z-10 will-change-transform shadow-xl transition-all duration-1000 ease-out ${
      ...
      {/* Primary Image (top left, overlapping) */}
      <div
        ref={primaryRef}
        className={`absolute top-0 left-0 w-[280px] h-[320px] md:w-[320px] md:h-[380px] lg:w-[380px] lg:h-[480px] overflow-hidden z-20 will-change-transform shadow-2xl transition-all duration-1000 ease-out ${
          hasAnimated ? 'translate-x-0 translate-y-0 opacity-100' : '-translate-x-20 -translate-y-20 opacity-0'
        }`}
