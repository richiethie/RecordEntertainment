...
interface LeftOverlappingImagesMobileProps {
  primaryImage: string;
...
export default function LeftOverlappingImagesMobile({
  primaryImage,
  secondaryImage,
  primaryAlt,
  secondaryAlt,
}: LeftOverlappingImagesMobileProps) {
  ...
  return (
    <div ref={containerRef} className="relative w-full h-[400px] overflow-hidden">
      ...
      <div 
        ref={overlayRef}
        className={`absolute bottom-8 right-12 w-40 h-[240px] bg-white/10 z-0 shadow-2xl backdrop-blur-sm will-change-transform transition-all duration-1000 ${
          hasAnimated ? 'opacity-100 scale-100' : 'opacity-0 scale-95'
        }`}
      ></div>
      
      {/* Secondary Image (behind, top left) - positioned to overlap with primary */}
      {secondaryImage && (
        <div
          ref={secondaryRef}
          className={`absolute top-0 left-16 -translate-x-[20px] w-[220px] h-[200px] overflow-hidden z-10 will-change-transform shadow-xl transition-all duration-1000 ease-out ${
      ...
      {/* Primary Image (bottom right, overlapping) - positioned to overlap with secondary */}
      <div
        ref={primaryRef}
        className={`absolute bottom-0 left-12 translate-x-[0px] w-[200px] h-[240px] overflow-hidden z-20 will-change-transform shadow-2xl transition-all duration-1000 ease-out ${
          hasAnimated ? 'translate-x-0 translate-y-0 opacity-100' : 'translate-x-10 translate-y-10 opacity-0'
        }`}
