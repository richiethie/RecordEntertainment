...
interface ClubFeaturesGridProps {
  features: Feature[];
}

export default function ClubFeaturesGrid({ features }: ClubFeaturesGridProps) {
  ...
  return (
    <section ref={sectionRef} className="py-8 sm:py-16 bg-dark-500 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-4 lg:px-6 relative z-10">
        ...
        {/* Mobile: Compact vertical stack, Desktop: Asymmetric Grid */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-6 lg:gap-8">
          {features.map((feature, index) => {
            // Create different card sizes for visual interest on desktop
            const isLarge = index === 1; // Middle card is larger on desktop
            return (
              <div
                key={index}
