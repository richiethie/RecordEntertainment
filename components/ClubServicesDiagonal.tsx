...
interface ClubServicesDiagonalProps {
  services: Service[];
}

export default function ClubServicesDiagonal({ services }: ClubServicesDiagonalProps) {
  ...
  return (
    <section ref={sectionRef} className="py-16 bg-dark-500 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-6 relative z-10">
          ...
          {services.map((service, index) => {
            ...
            return (
              ...
              >
                {/* Image Side */}
                <div 
                  className={`w-full mt-4 ${isEven ? '' : ''} flex ${!isEven ? 'order-2' : 'order-1'}`}
                >
                ...
                >
                  <div className="glass-card rounded-xl sm:rounded-3xl p-4 sm:p-8 hover:scale-105 transition-transform duration-500 relative overflow-hidden h-full flex flex-col justify-center">
                    {/* Green tint overlay - dark green, almost black */}
        ...
        {/* Desktop Layout - staggered side-by-side */}
        <div className="hidden lg:block">
          {services.map((service, index) => {
            const isEven = index % 2 === 0;
            // Add extra space after the second row (index 1)
